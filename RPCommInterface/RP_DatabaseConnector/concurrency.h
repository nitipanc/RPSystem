...
#ifndef RPCOMMINTERFACE_RP_DATABASECONNECTOR_CONCURRENCY_H_
#define RPCOMMINTERFACE_RP_DATABASECONNECTOR_CONCURRENCY_H_

#include <vector>
#include <RP_DatabaseConnector/account.h>
#include <RP_DatabaseConnector/statement.h>
...
inline void execute(const std::string &squery) {
    execute(squery, false);
}

extern handle insert(const std::string &query, bool keep_handle);
inline void insert(const std::string &squery) {
    insert(squery, false);
}

extern handle query(const std::string &query, bool keep_handle);
inline void query(const std::string &squery) {
    query(squery, false);
}
